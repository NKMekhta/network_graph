...
use eframe::egui::{self, Color32, DragValue, TextStyle};
use egui_node_graph::{DataTypeTrait, Graph, GraphEditorState, InputParamKind, NodeDataTrait, NodeId, NodeResponse, NodeTemplateIter, NodeTemplateTrait, OutputId, UserResponseTrait, WidgetValueTrait};
...
///
/// There will usually be a correspondence between `DataTypes` an`ValueTypes`es. But
/// this library makes no attempt to check this consistency. For instance, it is
/// up to the user code in this example to make sure no parameter is created
/// with a `DataType` of Scalar and `ValueType`pe of Vec2.
...
impl NodeTemplateTrait for MyNodeTemplate {
    ...
    fn build_node(
        ...
        match self {
            ...
            MyNodeTemplate::AddVector | MyNodeTemplate::SubtractVector => {
        ...
        if let Some(node) = self.user_state.active_node {
            if self.state.graph.nodes.contains_key(node) {
                ...
                ctx.debug_painter().text(
                    ...
                    if ctx.style().visuals.dark_mode { Color32::WHITE } else { Color32::BLACK },
