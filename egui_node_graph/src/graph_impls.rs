use super::{AnyParameterId, EguiGraphError, Graph, InputId, InputParam, InputParamKind, Node, NodeId, OutputId, OutputParam, SVec, SecondaryMap, SlotMap};
...
impl<NodeData, DataType, ValueType> Graph<NodeData, DataType, ValueType> {
    ...
    pub fn remove_connection(&mut self, output: OutputId) -> Option<InputId> { 
        self.connections.remove(output)
    }
    ...
    #[must_use] 
    pub fn connection(&self, output: OutputId) -> Option<InputId> {
        self.connections.get(output).copied()
    }
