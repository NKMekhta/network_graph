...
use eframe::egui;
use egui::{
    Area, Color32, Id, Key, Label, Layout, Order, Painter, PointerButton, pos2, Pos2, Rect, Response,
    RichText, Rounding, Sense, Shape, Stroke, TextStyle, Ui, vec2, Vec2,
};
...
use super::{
    ...
    Node, NodeDataTrait, NodeFinder, NodeId, NodeTemplateIter, NodeTemplateTrait, OutputId,
    SlotMap, UserResponseTrait, WidgetValueTrait,
};
    ...
    CategoryType: CategoryTrait,
{
    ...
    /// - `ui` must not be `None`
    /// - `all_kinds` must not be `None`
    /// - `user_state` must not be `None`
    /// - `prepend_responses` must not be `None`
    pub fn draw_graph_editor(
        ...
        if let Some(ref mut node_finder) = &mut self.node_finder {
        ...
        for response in &delayed_responses {
            match response {
                ...
                NodeResponse::CreatedNode(_) | NodeResponse::User(_) => {
                ...
                NodeResponse::DisconnectEvent { input: _, output } => {
                    let other_node = self.graph.get_output(*output).node;
                    self.graph.remove_connection(*output);
        ...
        if click_on_background && !cursor_in_finder {
    ...
    DataType: DataTypeTrait<UserState>,
{
        ...
        NodeData: NodeDataTrait,
    {
        ...
        if resp.drag_started() {
            if is_connected_output {
                let output = param_id.assume_output();
                let corresp_input = graph
                    .connection(output)
    ...
    fn show_graph_node(
        ...
        child_ui.vertical(|ui| {
            ...
            for (param_name, param_id) in inputs {
                if self.graph[param_id].shown_inline {
                    ...
                    if self.graph.connections.values().any(|i| i == &param_id) {
            ...
            .zip(input_port_heights.into_iter())
        {
            let should_draw = match self.graph[*param].kind() {
                ...
                InputParamKind::ConnectionOrConstant | InputParamKind::ConnectionOnly => true,
            };
